...
void fastMode(const uint8_t pin, bool val) {
#if defined(__AVR_ATmega328P__) || defined(__AVR_ATmega168__)
  if (pin < 8) bitWrite(DDRD, pin, val);
  else if (pin < 14) bitWrite(DDRB, (pin - 8), val);
  else if (pin < 20) bitWrite(DDRC, (pin - 14), val);
#else
  pinMode(pin, val);
#endif
}
